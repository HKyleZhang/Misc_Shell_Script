            ...
            do
              ...
              if [ "${ri}" -gt 1 ]; then clnum=$((clnum-1)); fi           
                 ...
                 if [ "${hyp}" = "H0" ]; then
                    ...
                    model=$(cat ${curdir}/${foldername}/${kclnum}/${foldername}_cluster${clnumfm}-out_bestrun/*iqtree | grep "Model of substitution:" | cut -d " " -f 4 )
                    iqtree -s ${seqfile} -m ${model} -t RANDOM -ninit 200 -ntop 50 -nt ${nth} -keep-ident -pre ${seqfname}.run -alrt 1000 -abayes
                    ((clnum++))
                 elif [ "${hyp}" = "H1" ]; then
                      iqtree -s ${seqfile} -m GTR+F+I+G4 -AICc -t RANDOM -ninit 200 -ntop 50 -nt ${nth} -keep-ident -pre ${seqfname}.run -alrt 1000 -abayes
                      ((clnum++))
                 fi
