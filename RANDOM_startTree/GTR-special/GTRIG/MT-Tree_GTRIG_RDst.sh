     ...
     cd ../run${iterindex}
       ...
       do   
         ...
         if [ "${wkfl}" -eq 3 ]; then
            ...
            modelsetting="${modelsetting0} -t RANDOM -ninit 200 -ntop 50 -nt ${nth} -keep-ident ${bsset}"
            iqtree -s ${file} ${modelsetting}
         elif [ "${wkfl}" -eq 5 ]; then
            modelsetting="-m GTR+F+I+G4 -AICc -t RANDOM -ninit 200 -ntop 50 -nt ${nth} -keep-ident ${bsset}" 
            iqtree -s ${file} ${modelsetting}
         fi        
       done
      ((iterindex++))
