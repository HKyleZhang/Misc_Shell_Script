      ...
      while [ "${iterindex}" -le "${iternum}" ]; do
        ...
        for iterfile in *; do
          ...
          if [ "${wkfl}" -eq 3 ]; then
            ...
            modelsetting="${modelsetting0} -ninit 200 -ntop 50 -nt ${nth} -keep-ident ${bsset}"
            iqtree -s ${file} ${modelsetting}
          elif [ "${wkfl}" -eq 5 ]; then
            modelsetting="-m MFP -AICc -ninit 200 -ntop 50 -nt ${nth} -keep-ident ${bsset}"
            iqtree -s ${file} ${modelsetting}
          fi
        done
        ((iterindex++))
