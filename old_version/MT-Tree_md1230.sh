     ...
     cd ../run${iterindex}
       ...
       do   
         if [ "${wkfl}" -eq 3 ]; then
            ...
            modelsetting="${modelsetting0} -t RANDOM -ninit 201 -ntop 50 -djc -o ZF -nt 1 -keep-ident ${bsset}"
            iqtree -s ${file} ${modelsetting}
         elif [ "${wkfl}" -eq 5 ]; then
            modelsetting="-m TEST -AICc -t RANDOM -ninit 201 -ntop 50 -djc -o ZF -nt 1 -keep-ident ${bsset}" 
            iqtree -s ${file} ${modelsetting}
         fi        
       done
      ((iterindex++))
...
##File sorting
     ...
     mv ../run${tnum}/*log ../run${tnum}/*gz ../${opname}/trivia
